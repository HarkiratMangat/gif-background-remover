...
| `--unprotect-region …` | Region-scoped BACKGROUND removal: inside the region the background key is re-applied and every protection decision is overridden, but non-background pixels are left alone. Says "this enclosed interior is background" — a white area inside a tower, the white inside a sparkle — which no protection flag can express. Unlike `--remove-region` it does not force-delete its box. `--recover-fade-alpha` ignores every PROTECTION flag but honours all three region flags (`--unprotect-region`, `--remove-region`, `--remove-region-track`) — corrected 2026-09-01; this used to say `--unprotect-region` was the only one, and a real session read that and hand-built a two-pass composite for something `--remove-region-track` does in one command. §48 |
...
| `--min-quality <n>` | A QUALITY floor for a `--target-kb` fit, the mirror of `--min-width`: rungs below `n` are removed so a stated range ("q65-85") is honoured as a requirement while resolution and frames are traded instead. A lossless rung is never excluded; APNG has no quality knob and is unaffected. `references/lessons.md` §48. |
