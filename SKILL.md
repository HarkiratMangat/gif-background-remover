...
**Skill version: v6.1.1** (previous: v6.1.0, v6.0.0, v5.5.0, v5.4.0, v5.3.0, v5.2.1, v5.2.0, v5.1.1, v5.1.0, v5.0.0, v4.0.0, v3.3.3 … v1). Full per-version detail, and the three-part versioning convention itself, live in `references/version-history.md` — read it when you need to know what a past version changed or which tier a pending change belongs to.

**v6.1.1** is a *correction* bump: two packaged files pointed at things the sandbox cannot open, and the gate meant to catch that could not see either of them.
