...
**Skill version: v5.5.0** (previous: v5.4.0, v5.3.0, v5.2.1, v5.2.0, v5.1.1, v5.1.0, v5.0.0, v4.0.0, v3.3.3 … v1). Full per-version detail, and the three-part versioning convention itself, live in `references/version-history.md` — read it when you need to know what a past version changed or which tier a pending change belongs to.

**v5.5.0** is a *minor* bump: a fifth pixel-art discriminator that finally reaches dithered and photographic art, plus three defects it exposed in the measures that were already there — two of which were recommending `--pixel-art` for ordinary antialiased vector icons — and a crash on static JPEG input.
...
```
**Read `appears_hard_edged` and `hard_edged_reasons` — not the raw numbers.** FOUR independent rules feed that verdict, any one of them is enough, and `hard_edged_reasons` names the one(s) that actually fired. The example above is real: that asset is pixel art with a band ratio of 20.895, *higher* than any genuinely antialiased asset measured, and it is detected anyway because a different rule carries it.

The four rules, in the order they were added:
