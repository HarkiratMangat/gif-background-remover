...
**Skill version: v6.2.0 — PENDING, not yet minted.** (last real released version: v6.0.0; last real tagged version: v6.1.0). Full per-version detail, and the three-part versioning convention itself, live in `references/version-history.md` — read it when you need to know what a past version changed or which tier a pending change belongs to. **A version is minted when a merge is approved, not per commit on a branch — do not bump this again until a merge is actually approved.** (This rule exists because this branch broke it once: `references/version-history.md`.)

**v6.2.0 (pending)** bundles everything on this branch since `main`: two new `--recover-fade-alpha` flags for a case named colours alone cannot express, an auto-detect fix, an erosion-calibration fix and a border-banding fix that all close real blind spots without any new flags, a fresh-eyes doc review, and three CLI-robustness fixes `/verify` found. A follow-up code review (medium effort) then found and fixed two real correctness bugs the initial implementation introduced.
- **`--fade-protect-region circle:cx,cy,r | rect:x,y,w,h` and `--fade-protect-colors <hex[,hex…]>`.** For a design element that shares a named `--fade-color` hue with something that genuinely fades but must never fade itself — there is no way to say "this hex fades here but not there" through colour alone, since it is the literal same palette entry in both places. Confirmed real case: a spinning icon reused its badge's outline colour and rendered with visibly inconsistent opacity across its own shape when it should always be fully opaque. The region force-opaques only confidently-art pixels within it (not the whole disc — an earlier version painted a visible solid disc over the region, including true background peeking through a concave gap in the protected shape), and `--fade-protect-colors` further restricts it to one named colour family (without it, the region also force-opaqued a different, legitimately-fading design element that happened to overlap the same area). **Both only mean something alongside `--recover-fade-alpha --fade-color <hex[,hex…]>` on a `.webp`/`.avif`/`.apng` output.**
- **Auto-detected fades (no `--fade-color` named) now absorb their own paler collinear stages too.** Previously that fix only ran behind an explicit `--fade-color`, so an unnamed fade with the same multi-stage structure could still freeze a paler stage fully opaque. No flag change — this makes `--recover-fade-alpha` alone more correct on its own.
- **A softmax blend across a pixel's WHOLE fading family (not just its best two matches) fixes border banding.** On the motivating asset, a traced neighbour-pixel jump fell from 100 alpha units to 76 and large jumps across the frame fell ~13%; visual inspection across 4 frames spanning the fade cycle confirmed no remaining banding artefact — the residual pixel-level differences a forensic scan still finds are real boundaries between different design elements (e.g. a fill colour meeting an outline colour), not an algorithmic gap. No flag change.
- **`--auto`'s erosion calibration now sees a halo it was previously blind to.** The old fringe metric only asked whether a ring pixel is closer to background than to art; a halo that is more than half-art but still an unresolved antialiasing mix never counted, so an asset like that stayed at erosion 0 no matter how visible the halo was. A second signal (is this ring pixel a genuine partial blend at all) is now combined with the first, and `--auto`/`--auto-erosion` pick up the fix automatically — no flag change.
- **`/verify` drove the two new flags with malformed/adjacent inputs and found three real gaps, all fixed.** A typo'd `--fade-protect-region` kind crashed with a raw Python traceback instead of a clean error — now caught and re-raised naming the flag and expected syntax. `--fade-protect-colors` naming a hex matching nothing in the palette silently protected the nearest family anyway — now applies the same 30-unit distance check `--fade-color` already had. `--fade-protect-region` without `--recover-fade-alpha` silently did nothing — now warns, mirroring the existing `FADE_EXCLUSIVE_FLAGS` warning pattern.
- **A fresh-eyes review (doc-coauthoring's reader-testing method, a real subagent with only the packaged files) found two real doc gaps**, both fixed: the description never mentioned resize/crop/shrink, risking a real "resize this sprite" request not triggering the skill; and progressive-disclosure trims (a dense `edge_hardness` evidence block condensed, a short inline compression-levers summary added) per `skill-creator`/`plugin-dev:skill-development` guidance.
