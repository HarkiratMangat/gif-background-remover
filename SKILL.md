...
**Skill version: v5.3.0** (previous: v5.2.1, v5.2.0, v5.1.1, v5.1.0, v5.0.0, v4.0.0, v3.3.3 … v1). Full per-version detail, and the three-part versioning convention itself, live in `references/version-history.md` — read it when you need to know what a past version changed or which tier a pending change belongs to.

**v5.3.0** is a *minor* bump: two confirmed recommendation bugs, both found because a README showcase was quietly demonstrating them.
...
**v5.2.1** is a *correction*: the frontmatter description was 1364 characters and **claude.ai rejects any description over 1024** — the upload failed outright. Rewritten to 929 with every trigger phrase kept, and `scripts/audit_docs.py` now enforces the limit. No local gate knew this constraint existed, so it surfaced only in the browser, after a merge, a tag and a package build.
