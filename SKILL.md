...
⛔ **A background that CHANGES COLOUR partway through the animation cannot be processed at all, and the wrong output looks perfectly healthy.** `--bg-color` is one value and detection reads one frame, so every frame after the change keys against a colour that is no longer on the canvas and keeps its whole background as opaque artwork. Measured on a 30-frame asset whose background runs magenta → pink → red → orange → yellow → green across frames 11–15: the frame-0 colour covers **2.37% of the canvas on frame 0 and 0.00% on frame 12**, so half the output frames keep their background — while the file is written, has plenty of opaque pixels, and passes every quality check the tool has. `--analyze` reports `background_color_stability` (with `changes: null` and an `unverified_reason` when the question could not be asked, never a quiet pass), `--recommend` returns `not_applicable_reason` instead of a command, `--auto` refuses, and `process` refuses independently for a run that never asked. **Split the animation at the colour change and give each run its own `--bg-color`, or re-export with a constant background**; `--allow-changing-background` keys the frame-0 colour anyway and accepts an opaque background on every recoloured frame. Scored on 71 real animated assets — 6 changing, 65 not — 6 of 6 fire and 65 of 65 never reach the rule at all. §35
