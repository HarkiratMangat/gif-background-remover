...
**Next action:** measure density on a dither-suppressed copy (small median filter, or count only changes persisting across several adjacent lines) and re-score against the same 37. The corpus and labels are in `local/Diors-builds Emojis/others/` with `LABELS.json` + `README.md`.
