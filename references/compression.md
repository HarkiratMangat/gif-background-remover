...
- **A common reason to want this combo:** `medium`/`heavy`'s dithering can look bad on fine vector linework (see the graininess note above) — if someone wants every frame kept AND no dithering artifacts, `optimize` (not `medium`/`heavy` + `--frame-stride 1`) is usually the cleaner single fix.
