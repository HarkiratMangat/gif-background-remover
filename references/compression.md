...
⚠️ **A DEEP downscale (below half) ranks BELOW frame-stride in that order, which reverses the older "frames go last" rule.** Measured on 640px flat vector art: downscaling to 75% made the file **2.3x LARGER**, and it stayed larger at 50% and 37.5%, because resampling invents intermediate colours and the art stops being flat. Frame-stride 2 at full resolution beat every one of those rungs on size AND kept the resolution. Moderate downscales (0.75, 0.5) still rank above frame-stride — only the drastic end moved. **`--pixel-art` keeps the original order entirely**: NEAREST resampling cannot invent a colour, and on the same asset its scale ladder is monotone (2/2/2/2/1 KB) where LANCZOS is not (2/34/30/25/18). The full rung table, the seven-target PRE/POST, and the mechanism are in `references/lessons.md` §42. If the fit log says `NOTE: downscaling made this file LARGER than full resolution`, that is this effect on your asset.

⚠️ **A byte cap is a CONSTRAINT; a stated resolution is a REQUIREMENT — `--min-width`, `--min-height` and `--min-dimension` express that.** The v6 trial delivered a 120x128 asset against an explicit "at least 128px wide": the fit walked down the scale axis and nothing stopped it. Each flag removes only the scale rungs that would breach it, so quality and frames are still traded freely, and if the target still cannot be met the failure message names the floor rather than quietly delivering a smaller file. A bare `--min-dimension N` constrains the **shorter side**, `min(width, height) >= N` — the stricter reading of "at least this big", and the right one for a square sticker or emoji slot; a 600x100 asset passes `--min-width 128` and fails `--min-dimension 128`. Aspect ratio is preserved throughout, so "width >= N, aspect preserved" needs nothing beyond `--min-width`. Set several and the tightest wins. The native rung is never removed: nothing here upscales, so a floor above the source is unreachable by definition.

⚠️ **The fit prints a COST ESTIMATE before it starts, and it is worth reading.** Up to 120 rungs, each re-encoding every frame, so the cost scales with frames x rungs and not with file count. Measured 2026-08-22: one 144-frame 640x640 asset took **207.84s** on 6 cores, and a first attempt died at a 120s tool timeout having produced nothing at all. Above roughly 5,000 frame-encodes the tool prints how many rungs, frames and serial batches it is about to work through. **If you are running under a tool timeout, render one file and one format per call** — and if the constraints are already known, pass the flags directly instead of running the search at all.
