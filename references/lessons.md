...
| Choosing `--edge-cleanup-erosion`, or `--auto` picked a surprising one | §19 (calibrated against the asset's OWN curve), §45.2 (a halo the old fringe metric reads as 0.0000 at every level) |
| A visible halo/fringe survives calibration, but the fringe metric reads clean | §45.2 (the ring pixel is more art than background, so the old signal never saw it) |
| An auto-detected fade (no `--fade-color`) leaves a ghost boundary or freezes a paler stage opaque | §45.1 (absorption only ran behind a named hex) |
| Adjacent pixels on a fading border step instead of ramp, even after the top-2 blend | §45.3 (which TWO members rank best flips between neighbours in a big family) |
...
⚠️ **REVERSED 2026-08-23 — hurricane's ladder was not a hand-picked-tint chain to detect and decline; it was THREE independently-fading regions the flag-based path could not see, and the fix is absorption, not detection.** Harkirat supplied the fact that broke the earlier conclusion: hurricane's badge has three fading colours (`#DA4B85` left half, `#FD6252` right half, `#042A75` outline), not one, so a single `--fade-color` anchor left two of the three families frozen solid — the "ghost octagon" seen at frame 39, the asset's own documented worst tumble-risk frame, was the outline's paler stages sitting unrecognised in the ladder the same way the original tint-chain theory described. Fix, in `recover_fade_alpha_frames`'s `fade_hexes` branch: absorb every palette candidate near-collinear (cosine ≥0.97) with a *named* fading colour, and paler than it (closer to background), into that colour's fading family — propagated into both `protect_parents` (so absorbed members do not get re-merged away) and the final `fading` set (a first version of this fix updated only `protect_parents`, which is itself a bug: the family survived as distinct palette entries but rendered fully opaque anyway because nothing told the unmix step they were fading). Verified clean on hurricane across frames 30-51 (the shrink/regrow window spanning frame 39) with zero pinwheel-icon damage across all 120 frames.

**Corpus-validated 2026-08-23 against the exact four real `--recover-fade-alpha` assets in the table above** (`crystal`/`gift`/`love`/`heart` — the ones this code area has a documented regression history on, §16's reverted "saturation promotion" being the prior cautionary tale). Forced `--fade-color` on each with the pre-fix and post-fix script, diffed every frame's alpha plane exactly (not a visual comparison). `crystal`/`love`/`heart`: no palette entry met the absorption threshold, byte-identical output. `gift`: one entry (`fa7fa0`) DID get absorbed — cosine 0.98 with the named `fd6050`, a real change to the "fading colours" log line — but the rendered alpha was byte-identical across all 172 frames regardless, because `fa7fa0` never appears as a background-blend anywhere in the animation (constant 4,303-8,425 px per frame, never trending toward 0 the way a fading count does), so it is always assigned t≈1.0 whether or not it is flagged as fading. **0 of 461 combined frames differ from the pre-fix render on any of the four**, so the near-collinear absorption threshold that fixed hurricane does not regress the assets that motivated the original ladder-detector caution. Not yet extended to `detect_fading_colors`'s auto-detect path — this fix only reaches pixels behind an explicit `--fade-color`. ✅ **RESOLVED 2026-08-24** — extended to the auto-detect branch too, anchored on each auto-detected colour's own pre-absorption colour. See §45.1.

⚠️ **Two more real defects in this same fix, both found by Harkirat looking at the actual render, neither by any automated check — same day, 2026-08-23.** He rejected the first delivery outright once he saw it ("i do not want the frame 53 compromise") and asked for the whole class solved, not a partial fix kept as a known limitation.

**Defect A: a wall made of the fading family's OWN colours can seal off part of the fading family's OWN interior.** `recover_fade_alpha_frames`'s protection is topological — an enclosed pocket defaults to protected/opaque, which is exactly right for crystal.gif's white interior (colorimetrically identical to real background, protected ONLY by staying enclosed) — but the barrier had no notion that an enclosing wall can itself be part of the SAME fading family as what it encloses. Two distinct mechanisms produced this, found by tracing the exact sealed pixels rather than pattern-matching to a theory formed on the first one:
- A thin ring where two ART colours meet (the outline touching the fill) reads as "unexplained" (`res > FADE_RESIDUAL_TOLERANCE`) because `unmix_against_palette` only tests ONE colour blended with the BACKGROUND, never an art-to-art blend. New `fading_seam_mask()` makes such a seam permeable for flood-reachability when BOTH sides are named fading colours — a provable no-op whenever fewer than two colours are named (every one of the four corpus assets above).
- A pocket sealed only by non-solid barrier pixels (i.e. no `solid & t >= FADE_BARRIER_ALPHA` pixel anywhere on its wall) gets re-tested for reachability using ONLY genuinely solid barriers. If reachable that way, AND under 50% of the pocket is background-degenerate (`t < 0.05` — 97%+ on crystal's real protected interior, under 8% on hurricane's real fading pockets), AND the pocket is at least 50px (excludes dozens of 1px AA specks found right on crystal's sparkle edge that individually pass the background-fraction test trivially and, promoted, left a thin mis-coloured rim — caught by a real pixel diff, not assumed safe), it is exposed.

Two rejected attempts before this, kept here because both are real, plausible-looking and both failed a real corpus check rather than being caught by inspection: a version gated on `art_prior` (pixel-POSITION frequency, reusing the existing opaque-fading-block rule's signal) broke crystal for real — 13,913px in one frame — because crystal's own outline shifts position slightly frame to frame too, the identical problem as hurricane's spinning icon; a position-frequency prior cannot tell "nothing protected lives here" from "protected content lives here, just not always in the same spot." Reverted the same turn it was measured, not left in the branch to be cleaned up later.

**Defect B: the 0.97 cosine floor from the fix above was too strict for a bevel highlight on the SAME named outline.** Found after Defect A shipped, by Harkirat again ("some frames show the navy outline flashing white or part of it flashing/turning white while the other part continues to fade"). Traced to hurricane's outer octagon BORDER (a separate design element from the inner pinwheel outline, both drawn in the `042a75` family) having its own bevel-highlight shade at cosine 0.9607 with the named anchor — under the 0.97 floor by a narrow margin, so it stayed classified solid and rendered fully opaque (one side of the border) while the rest of the border faded normally (the other side, cosine 0.98, correctly absorbed). Widened to 0.95, still comfortably above the 0.762–0.93 ceiling this asset's own genuinely-distinct real colours measured at.

**Both defects corpus-validated the same way, at every stage, including the false start:** crystal/gift/love/heart rendered with `--fade-color` on the pre-fix and post-fix script each time, alpha planes diffed exactly. 0/461 frames differ after Defect A's real fix; 0/461 still after widening the threshold for Defect B (this loop is NOT gated by colour count the way `fading_seam_mask` is, so it genuinely needed re-checking, not just an assumption it was still safe).

✅ **Defect C investigated 2026-08-24 — the border-banding described below got a real but partial improvement; the investigation's main outcome was finding and fixing a DIFFERENT, more severe defect on the SAME asset.** `unmix_top2_against_palette` plus a softmax(-residual/`FADE_BLEND_SOFTMAX_T`) blend between a pixel's best and 2nd-best match, applied whenever both are named fading colours, with no hard on/off gate (a gated first version was measured to barely move the outcome — a boundary blend only softens the seam between two flat regions, it cannot smooth a whole arc the source GIF painted as one flat shade). Mixed result on the border specifically (2 of 4 sampled frames improved, 2 slightly worse) — kept anyway since it is corpus-safe and a real, if small, improvement, but the border-banding limitation described below is still genuinely open in the strict sense.

⚠️ **What the investigation actually found, from Harkirat's own annotated screenshot: hurricane's inner pinwheel/vortex reuses the SAME `#042a75` family as the outline, and should NEVER fade — always fully opaque, like the centre hub already correctly renders — but was showing mixed opacity across its own blades.** Five broken attempts before the real fix, kept here because each is a distinct, real root cause: (1) grouping same-family `exact` pixels into one connected component and setting ONE alpha via a low percentile, first version with no cross-family restriction, merged the pink fill, orange fill and navy outline into one 273,222px blob (nearly half the canvas) whose own true silhouette edge dragged the whole thing to alpha 0; (2) restricted to one NAMED anchor family via nearest-EUCLIDEAN distance — wrong test, since a very pale colour sits numerically close to any anchor regardless of hue, so real pale-navy members (`#9078a7`, `#ceb7c3`) were assigned to the pink family anyway, reproducing the same blob; (3) fixed the family test to COSINE similarity against each anchor's own ray from background — matching how the absorption loop itself defines a family — correctly isolated the pinwheel (34,346px) as its own component, confirmed by adding real instrumentation (`fade_debug`, later removed) to the function itself, after an external reconstruction of the same state stopped matching the real output three times running; (4) even correctly isolated, "one uniform alpha per component" targets the wrong goal ("uniformly faded") against the real requirement ("never faded"), so the whole percentile mechanism was abandoned and removed rather than patched further, including its `FADE_FAMILY_*` constants — the real fix needed none of it.

**The actual fix: two new general-purpose flags.** `--fade-protect-region` (circle/rect spec, reusing `parse_protect_regions`) force-opaques a manual region at its own source colour, restricted to pixels that are confidently art (`res<=FADE_RESIDUAL_TOLERANCE & t>0.02`) so the region does not also force-opaque true background peeking through a concave gap in the protected shape — a first version without this restriction painted a literal white disc over the whole region. `--fade-protect-colors` further restricts the region to one named colour family (via the same `family_of` cosine assignment) — without it, the region force-opaqued the badge's own translucent FILL too (since the fill legitimately overlaps the same radius), painting a visible pale disc from the fill's already-faded source colour. Verified across 8 sampled frames including a structurally anomalous tumble frame (40, where the border itself fragments into several disconnected pieces): vortex alpha is 255 on every art pixel every frame, background correctly shows through the pinwheel's own concave gaps (alpha 0, ~13,700px every frame, unchanged), and the border keeps fading normally in the same region. Corpus-validated at every single step of this whole investigation, including every broken intermediate version: crystal/gift/love/heart, forced `--fade-color`, exact alpha diff, 0/461 combined frames differ, every time.

Two things checked and confirmed PRE-EXISTING, not introduced this session (present identically in the pre-session baseline): a small yellow sparkle near the border at frames 59-60 (Harkirat confirmed real intentional art, not a bug), and the border legitimately fading all the way to zero opacity for several consecutive frames (42-45) as a smooth, continuous, non-anomalous part of its own fade cycle (measured: alpha 71→60→50→40→30→21→9→4→0→0→0 across frames 35-45, a monotonic decay, not a discontinuity) — Harkirat's call was not to spend further budget on this specific edge case tonight.

**The strict border-banding defect itself is unchanged from the description below and still open — ⚠️ AS OF THIS WRITING (2026-08-23); CLOSED the next day, see the correction after the description below.** The octagon border still bands rather than gradates smoothly: sampled at 15° steps around the ring on frame 35, alpha steps 43 → 50 → 93–96 → 113–115 → 212 rather than ramping, because `unmix_against_palette` matches every pixel to whichever SINGLE palette colour fits best (`k = r2.argmin(1)`) and several genuinely-distinct pale shades sit close together in this part of colour space — each renders at its own correctly-computed alpha, but adjacent pixels snap between different winners instead of blending. **Unlike Defects A and B, this is not a miscategorization a threshold or a permeability rule can close** — every shade here IS being unmixed correctly against its own true best match. A real fix means changing HOW pixels are matched (a soft/weighted blend across the k-nearest fading-family entries, or a proper multi-colour barycentric unmix instead of single-nearest-line projection), which is a change to `unmix_against_palette`'s core, not a bounded fix to `recover_fade_alpha_frames`'s barrier logic the way A and B were — and it needs its own corpus validation pass, not an assumption that the same crystal/gift/love/heart check still applies. This session's softmax blend is a real but small step toward it, not a fix. Start the next session here.

✅ **RESOLVED 2026-08-24 — the k-nearest blend this section called for was built, and closed the defect.** `unmix_top2_against_palette`'s two-member softmax generalised into `unmix_family_blend`, summing over the WHOLE fading family rather than just the best two (§45.3). The traced pixel jump shrank further (22→97, was 22→122) and a broader scan found ~13% fewer large neighbour jumps — but pixel forensics alone still found ~2,100 "unexplained" jumps in frame 35's mid-alpha region. What actually closed it was rendering the real output, compositing over a checkerboard, and looking: no jarring banding at any fade depth checked, and the remaining traced pixels are genuine antialiasing between two different design elements (the pink fill meeting the navy border), not within-family ambiguity. The k-nearest/barycentric redesign this paragraph called for was not needed. Full closure record: §45.3.
...
⚠️ **This section records a measurement and TWO REJECTED FIXES, both against the CALIBRATION rule.** It is written down so the next attempt starts from the evidence rather than from the same two ideas. **The calibration side was later closed a different way (§37.4, a search-space cap, not a cost term) and a second, independent mechanism — a small KEPT component being erased rather than a bad level being picked — was found and fixed separately (§37.9).** Some residue remains on the population that motivated this section (§37.9's own measured numbers), so this is not a claim of total closure — read §37.4 and §37.9 before assuming the defect below is still exactly as first described.
...
⚠️ **The obvious render gate was VACUOUS for this, and saying so is the more useful lesson.** The 106-asset render baseline renders every asset through `--auto`, which already calibrated. It came back **0 changed of 230 records** — which partly means *this set cannot express the change*. The population the change can reach is the MANUAL path, and measured there over 39 assets (`labelled` + `trial`) — ⚠️ **which is itself only part of what the change reaches**, since roughly 265 further assets in `emoji` and `small_aa` qualify and were not rendered; `sprites` and `alphas` are excluded by the calibrator's own guards, not by the sample. Stated rather than glossed, because a gate scoped to the population you happen to have is the failure this very section is about: fringe improved on 10 (up to −2,859 px on the worst frame), unchanged on 27, and the artwork cost was **0.49–0.95 × the artwork's own perimeter** on every asset but one — less than a single 1px ring, which is exactly what erosion 1 should cost. A fraction is not damage until you check what it is a fraction of. The one outlier calibrates to erosion 2 through the convex-curve rule (§37.10) and loses 1.89 × perimeter; rendering it under the PRE code with `--auto` produced a byte-equivalent result, so that outcome is an existing `--auto` behaviour now reaching the manual path, not a new one.
...
⚠️ **The tension with full autonomy is real and is not smoothed over.** An unattended run has nobody to ask, so the question is made **answerable in advance** rather than dropped: `--assume-protect`, `--assume-remove`, `--assume-no-fade`. A run that is neither pre-answered nor willing to be asked stops; a pre-answered run PRINTS that it acted on an assumption, because an assumption that leaves no trace is indistinguishable from a measurement. **Measured refusal rate before shipping it, because a refusal that fires on everything is correctness bought by making the feature useless: 39 of 304 assets, 12.8% pooled** — 29 enclosure-only, 8 fade-only, 2 both; emoji 19.7%, trial 12.5%, labelled 12.9%, small_aa 7.0%. ⚠️ **The first figure taken was 10.2%, and it was STALE IN THE UNSAFE DIRECTION** — measured before the fade question existed, so it counted one of the two refusals and understated how often `--auto` stops, most sharply on small icons (3.5% against the true 7.0%). It had already been written into three packaged files before a falsification pass caught it. **A rate measured against a gate is invalidated by adding a second gate**, and nothing warns you: both numbers are real measurements of something. And both questions arrive in ONE refusal: an unattended caller pays a whole tool call per refusal, so asking serially turns two questions into two lost calls.
...
## 45. Auto-detected fades never absorbed their own family; galaxy's erosion metric read zero at every level; and blending only the top two family members just relocates the border's cliff

Three separate continuations of the gate-8 trial's open items (§34, item 3's border banding; item 6, the erosion detector), done in one session 2026-08-24 because all three touch the same fade-recovery/edge-quality machinery. §45.1, §45.2 and §45.3 are all closed; §45.4 is a follow-up code review that found and fixed two real bugs (and one test-coverage gap) in this same work — read it after 45.1-45.3, not instead of them.

**Also searched as:** self-fading auto-detect · unnamed fade absorption gap · fringe metric blind spot · halo not caught by calibration · n_fraction · blend_pixel_fraction · k-nearest unmix · Voronoi ray boundary · multi-colour ladder blend

### 45.1 The collinear-family absorption fix only ever ran behind a named `--fade-color`

§34.2's absorption loop (`recover_fade_alpha_frames`'s `fade_hexes` branch) folds a palette candidate into a named anchor's family when it is near-collinear (cosine ≥0.95) with, and paler than, that anchor — this is what stopped hurricane's outline from freezing its own paler stages solid. It lived ONLY in the `if fade_hexes:` branch. The `else:` (auto-detect, `detect_fading_colors`, no `--fade-color`) branch built its `parents` list straight from whatever `detect_fading_colors` flagged and never absorbed anything — so an auto-detected fade with the exact same multi-stage structure got none of the fix, reproducing the identical ghost-boundary bug on any asset a user runs without naming a colour by hand.

**Fix:** the same cosine-≥0.95 loop, copied into the `else:` branch, anchored on `_auto_anchors` — a snapshot of each auto-detected colour's OWN pre-absorption colour, taken before the loop mutates `parents` — instead of the named `want` list. `family_of` (used by `--fade-protect-colors` and the family-blend below) was generalised to read from `_family_anchors = want if fade_hexes else _auto_anchors`, one cosine-nearest-anchor computation shared by both branches instead of two.

Corpus-validated on crystal/gift/love in pure auto-detect mode (`fade_hexes=None`, before vs. after, exact per-pixel diff): crystal and love come back byte-identical — their single auto-detected anchor (`fdcb50`) has no near-collinear unclaimed candidate in `provisional`, so the new loop is a genuine no-op there, exactly as `unmix_family_blend`'s "strict generalisation" claim in §45.3 predicts. Gift genuinely changes: `fa7fa0` — the SAME entry the named-hex case already validated absorbing safely (§34's hurricane fix) — now also gets absorbed on the auto-detect path, moving 36,990 alpha px across 77 of 172 frames, every one of them 1-3 alpha units (min 3, max 32, mean 35.6 vs 33.9 before) — a small boundary-region refinement from `fa7fa0` correctly joining `fd6050`'s fading family in the flood-barrier/seam logic, not a dramatic reclassification. Harness suite (`pytest scripts/harness`) unchanged: the same 2 pre-existing failures (the destroyed-fixture tests from the accidental `rsync`, §34's own record), 0 new.

### 45.2 galaxy.gif's halo, and every asset like it, was invisible to the ONLY signal `calibrate_edge_cleanup_erosion` had

The erosion calibrator (§19) picks a level by minimising `measure_outer_ring_background_fraction`: for the near-opaque outer ring, what fraction of pixels are closer to the BACKGROUND colour than to any art colour. That is a narrower question than "is this pixel a clean flat colour" — a halo pixel that is MORE than half art (`t > 0.5` in unmix terms) never counts, because it is closer to art than to background by definition, even though it is visibly an unresolved antialiasing mix the erosion should have eaten. Measured directly against the real `--auto --edge-cleanup-erosion N` output (N=0,1,2, not a reconstruction): **galaxy.gif reads 0.0000 on this signal at EVERY candidate level.** The calibrator's own floor-plus-tolerance logic then has nothing to act on — level 0 already "meets the floor" — so galaxy stayed at erosion 0 no matter how visible the halo was to a human. This is the item filed since the original gate-8 trial (§34, "erosion-calibration/haloing on flat vector art — galaxy/secure/broadcast/notification, the fringe metric reads 0.0 at every level") and left open because the fringe metric itself needed a second, different signal, not a threshold tweak.

**The new signal, `measure_outer_ring_blend_fraction`:** for the same near-opaque outer ring (widened to a 2px dilation, since a leftover antialiasing rim is not always 1px deep), unmix every ring pixel against the asset's own palette and ask whether it is a genuine PARTIAL blend — `res` within `FADE_RESIDUAL_TOLERANCE` (well-explained as a background/art blend at all) AND `t` strictly between 0.08 and 0.92 (neither confidently background nor confidently art). This is the exact vocabulary `detect_fading_colors` already uses for "this is a translucent stage, not a flat colour" — one definition of "blended", not two independently-tuned ones. Measured on galaxy: **0.5989 → 0.1359 → 0.0003** at erosion 0/1/2, a clean monotone signal the old metric could not produce at any level.

**Combining, not replacing:** `calibrate_edge_cleanup_erosion` now takes the per-frame MAX of the two signals before averaging. Neither subsumes the other — background-fraction still catches a pale fringe genuinely closer to background than to any art colour, which the blend signal's narrower `t`-range test does not directly test for. Confirmed against real end-to-end CLI runs (the `erosion calibrated against this asset's own curve (...) -> N` log line, not a probe of the internal function): **galaxy now correctly selects erosion=1** (was 0). The two already-correctly-calibrated controls, `rocket.gif` and `secure.gif`, land on the SAME erosion=1 they already got before this change — confirmed via the identical real log line before and after, not assumed from the formula. `broadcast.gif`'s own real defect (an enclosed-region classification issue, unrelated to erosion, root cause still open — §34's item 8) is untouched, as expected. `notification.gif` and `megaphone.gif` could not be probed at all in this pass: both now hit `--auto`'s v6.1.0 refusal gates (a nameable fade, a partial enclosure) before erosion calibration is ever reached — an orthogonal, pre-existing gap in a different part of the pipeline, not something this change causes or was expected to fix.

### 45.3 The top-2 border blend was real and safe, and still not enough — the family has 9 members, not 2

§34's softmax blend between a pixel's best and 2nd-best fading-family match (`FADE_BLEND_SOFTMAX_T`) was shipped as "a real but small improvement, not a fix" for hurricane's octagon border stepping. Traced on TWO REAL ADJACENT source pixels (frame 35, `(203,41)` and `(204,41)`) with real instrumentation, not a reconstruction: both individually blend correctly under the top-2 rule (22 and 122 respectively, matching the softmax formula exactly when checked by hand), but they arrive there via DIFFERENT winning pairs — `(203)` blends `db4b86`+`c7939e`, `(204)` blends `ceb7c3`+`c7939e` — because hurricane's absorbed navy-outline family has **9 members** (`db4b86, fd6050, 052a75, c7939e, 2f377d, 5a4f8c, 816d8e, 9078a7, ceb7c3`), not 2, and which two rank "best" is itself a discrete Voronoi-style decision that flips between neighbouring pixels. Limiting the blend to the winning pair does not remove that discreteness, it just relocates the cliff to sit at the pair's own boundary instead of the raw single-winner's.

**Fix:** `unmix_top2_against_palette` (dead after this change, deleted) generalised into `unmix_family_blend` — the SAME softmax(-residual/T) weighting, summed over EVERY fading-family member instead of just the best two. A residual far past `FADE_RESIDUAL_TOLERANCE` earns essentially zero weight regardless of family size, so this is a strict generalisation: with a single fading colour (crystal/gift/love/heart's usual case) it reduces to plain single-ray unmixing, and with exactly two close members it reduces to the old top-2 blend, both confirmed by corpus diff below. On the SAME traced pixel pair the step shrank from 22→122 (Δ100) to 22→97 (Δ76) — real, ~30% smaller, not eliminated. A broader scan of frame 35's mid-alpha region (10<alpha<245) for neighbour jumps >60 units found the count drop from 2,460 to 2,149 (~13% fewer) horizontally.

**Why it looked like it might not be a full fix, and why it turned out to be one.** The pixels that still jump were checked individually: `(41,204)`'s best single-ray fit is `res=6.61` (ceb7c3) — six times worse than `(41,203)`'s `res=1.08` (db4b86) — meaning no single ray explains `(204)` nearly as tightly as its neighbour does. That signature (a locally poor fit versus a locally excellent one right next door) is consistent with genuine antialiasing between TWO DIFFERENT DESIGN ELEMENTS (the pink fill touching the navy border) rather than within-family ambiguity a wider blend radius can smooth — a legitimate boundary between two things that are each fading independently, not a single ray rendered with a discontinuity.

⚠️ **CLOSED 2026-08-24 (same session, continued) — visual inspection, not just pixel forensics, is what actually settled this.** Rather than keep tracing individual jump pixels by hand (which cannot tell "still a bug" apart from "correctly rendering two different elements meeting" at scale), the real render was composited over a checkerboard and inspected directly at four frames spanning the fade cycle: frame 0 (fully opaque baseline), frame 35 (the original motivating frame), frame 39 (the documented worst tumble-risk frame), and frame 78 (mid-tumble, heavy fade). None show jarring banding — the octagon border reads as a smooth, uniformly-toned translucent ring at every fade depth checked; the pink/orange split down the badge's middle is genuine two-colour design (not a rendering artefact); the vortex stays correctly fully opaque throughout. Zoomed 4x crops of the specific traced boundary (the `(203,41)`/`(204,41)` region) show an antialiased diagonal edge between the purple border band and the pink fill — a real, intentional boundary between two elements, not a stepped artefact. **The remaining ~2,100 pixel-level "jumps" found by the earlier forensic scan are this: legitimate multi-element and multi-facet boundaries, not an algorithmic gap** — the family-blend generalisation (this section) is the actual, complete fix for the ORIGINAL defect (single-family banding along one gradient), and the k-nearest/barycentric redesign flagged below turned out not to be needed. Kept the entry point description below for the record, but it is no longer live work.

The entry point that would have been needed, had visual inspection found a real remaining defect: `unmix_against_palette`'s own single-nearest-line match (`k = r2.argmin(1)`) — a proper k-nearest or barycentric unmix, not another blend-radius or softmax-temperature tweak — using the SAME real-adjacent-pixel tracing method plus a real rendered-and-viewed composite (not pixel statistics alone) before trusting any conclusion about what is actually happening at a given border pixel.

**Corpus-validated, forced `--fade-color`, exact alpha diff, before vs. after this change alone:** crystal and love — 0 alpha pixels differ across both (byte-identical `t`), only ≤1-unit RGB rounding noise from the reordered float math (14,952 / 220,984 px respectively, mean delta 0.34-0.37, max delta 1) — exactly what "strict generalisation, single-member family" predicts. Gift — a REAL change (it has the 2-member `fd6050`+`fa7fa0` family §45.1 also touches): 19,805 alpha px differ across 96/172 frames, delta distribution median 2, mean 2.65, max 32, zero pixels above 60 and zero 0↔255 flips — a bounded smoothing refinement, not a new failure mode. Full 106-asset render-diff gate run TWICE, isolating each layer: §45.1+45.2 alone against the pre-session baseline showed only `galaxy.gif` (the expected, intentional erosion-detector fix — opaque pixel count drops ~3.7-3.9%, matching the halo removal) and two already-flagged-`unsuitable_no_edges` sprite files (0.3-1.0% opaque-count change, plausibly the SAME fade-absorption path triggering slightly differently on a borderline auto-detected case) changed, 0 crashes, 0 unexpected returncodes across all 230 records (196 successful renders, 34 expected refusals). §45.3 (family-blend) added on top showed **0 of the same 230 records changed at all** — not evidence of nothing, but the SAME structural blindness noted for the corpus-score gate on §45.1/45.2 (`feedback_metric_that_excludes_your_change`): this population's assets reach `recover_fade_alpha_frames` only via plain `--auto`, which does not choose to invoke fade recovery on gift/crystal/love/hurricane the way the direct, forced-`--fade-color` corpus check above does — so this gate cannot see the family-blend change either way, and the render-diff numbers here should not be read as validating §45.3. The forced-`--fade-color` corpus diff two paragraphs up is what actually exercises it.
### 45.4 A code review of §45.1-45.3 found two real bugs and confirmed one wasn't there

An 8-angle review (2026-08-24, same session, `/code-review medium effort`) against `main...HEAD` found and fixed: `unmix_family_blend` (§45.3) summed softmax weights across the WHOLE family with no floor on the per-member `dd` denominator, so a family member injected via `force_include` (a named `--fade-color`, or an absorbed collinear stage) that lands at or near the background colour produced `NaN` for the WHOLE pixel's blended output -- unlike the single-winner `argmin` path it replaced, which naturally skips a `NaN` candidate in favour of any real one, summing does not. Fixed by flooring `dd = np.maximum((d*d).sum(1), 1e-6)`: a degenerate member then correctly earns a huge residual (near-zero softmax weight) instead of poisoning the sum. `--fade-protect-colors` (afternoon session, this branch) matched the user-typed hex to a palette entry via raw EUCLIDEAN distance, then read THAT entry's (correctly cosine-computed) family -- reproducing, at the user-facing boundary, the exact pale-colour-nearest-wrong-anchor bug `family_of`'s own cosine fix exists to prevent internally. Directly demonstrated on hurricane's real palette: `#9078a7`/`#ceb7c3`/`#816d8e` are cosine-family navy (matching `family_of`) but Euclidean-nearest to the unrelated pink anchor. Fixed by extracting `_nearest_anchor_by_cosine` as a shared helper and using it for BOTH `family_of` and `--fade-protect-colors`'s family assignment, keeping the Euclidean check only for its original purpose (does the typed hex match a real detected colour at all).

The review also flagged, and this pass fixed, three real cleanup issues: the collinear-family absorption loop (§45.1) was duplicated near-verbatim between the named and auto-detect branches -- extracted to `_absorb_collinear_paler_stages`, called from both; `unmix_family_blend` was being run over the WHOLE frame inside the render loop even though only the (typically small) `blend`-masked subset of pixels ever uses its result -- restricted to `rgb[blend]` before the call; and `calibrate_edge_cleanup_erosion`'s new dual-signal loop (§45.2) recomputed a full-image `unmix_against_palette` pass once per (candidate, frame) even though the unmix result is invariant across candidates -- fixed with a LAZY per-frame cache (not an eager precompute: a first version precomputed unconditionally at the top of the function and crashed every erosion-calibration test, whose synthetic fixtures pass `(4,4)` frames with no colour channel at all -- the eager version ran `unmix_against_palette` before any candidate's ring had a chance to short-circuit it via the existing empty-ring check; the lazy version only computes on the first candidate whose ring is actually non-empty for a given frame, which for the synthetic fixtures is never, so the real function's own early return protects it).

One review candidate was investigated and found FALSE on direct execution: a claimed `NameError` (`want` undefined) at the `family_of` anchor-selection line was refuted by actually running `--recover-fade-alpha --fade-color <hex>` end to end -- it succeeds. The reviewer agent had found `_want` (a different, earlier-scoped variable in the absorption loop) and missed the LATER, separate `want = [...]` assignment inside the `fading`-set-construction block that the flagged line's own `if fade_hexes:` guard is paired with. Kept as a reminder that an agent's `rg`-based trace of a large function is not the same as running it.

**Corpus-validated, both bugs' fixes plus all three cleanup refactors together:** forced `--fade-color` on crystal/gift/love -- byte-identical to the pre-review-fix state on all three (0 alpha, 0 rgb diff), confirming the fixes and refactors change nothing for the non-degenerate, non-mismatched cases these three assets exercise. Pure auto-detect (no `--fade-color`) on the same three -- crystal/love unchanged in alpha (RGB max delta 1, float-reordering noise); gift shows a real but bounded change (alpha delta min 1/max 4/mean 1.53 over 605 px) that is the FIRST validation of §45.3's family-blend generalisation specifically in auto-detect mode (the earlier §45.3 corpus check only forced `--fade-color`) -- small and consistent with every other family-blend delta measured this session, not a new failure mode. Full `pytest scripts/harness` clean: same 2 pre-existing failures (the destroyed-fixture tests), 0 new, including two new tests added directly for the coverage gap the review found (`test_blend_fraction_rescues_an_asset_the_background_fraction_signal_cannot_see`, `test_blend_fraction_signal_does_not_disturb_an_already_correct_background_fraction_pick`).

## 46. A doc fix for the --analyze/--recommend redundancy did not stop it recurring — the guarantee moved into the tool

**Also searched as:** duplicate tool calls · wasted turns · redundant analysis · called both modes · agent efficiency · a discipline is not a control · in-tool reminder · nagging note · stderr warning · workflow rule ignored · doc read once not retained

The 2026-08-19 three-agent trial measured 50-74 tool calls on a five-asset job and named one cause: sessions calling `--analyze` then `--recommend` on the same file, paying for the identical computation twice, because `--recommend`'s JSON already embeds `--analyze`'s full report under an `"analysis"` key. SKILL.md documented the rule the same day. **The 2026-08-23 trial found three independent fresh sessions doing the exact same thing anyway, on all 10 assets** — the rule had been read, apparently, and was not retained across nine repeats of the same choice on the same job.

**This is the class of failure [[feedback_a_discipline_is_not_a_control]] already names: if avoiding a waste requires an agent to remember a paragraph before acting, the paragraph is not a control.** A prose rule has to be read, retained, and recalled at the exact moment the mistake is about to happen — three separate conditions, any of which can fail under task pressure. A note printed by the tool itself, in response to the exact action being taken, only needs the agent to read output it already receives.

**The fix: `--analyze` now prints a one-line reminder to stderr on every standalone call**, naming `--recommend`'s redundant coverage directly. `_ANALYZE_RECOMMEND_NOTE` is a module-level constant, printed once per process regardless of whether the call is single-file or the `run_read_only` multi-file path — no per-asset repetition, since a multi-file job would otherwise print the same line N times, the exact "read the same paragraph five times" complaint the format-ranking dedup (`_FORMAT_RANK_EMITTED`, same file) already exists to avoid. It cannot catch the FIRST redundant call an agent makes on a job's first asset — nothing can, short of refusing to run `--analyze` at all — but it can break the pattern from repeating across assets 2 through N of the same job, which is the actual shape the 2026-08-23 trial measured (all 10 assets, not just one).
...
- ⚠️ **`--target-kb` is a CEILING and never raises quality into spare headroom.** When the first render already fits, no fit runs and the file keeps the `--avif-quality` default of 70. Measured on the 192px grenade: `--target-kb 250` alone delivered **99.8 KB at q70**, leaving 150 KB of the cap unspent, where the hand-rolled search had correctly found q85. **On a stated range, start the run at the TOP of it** (`--avif-quality 85 --target-kb 250 --min-quality 65`) — the ladder only ever walks down from where you start it. This is the one axis on which the hand search beat the tool, and it is worth recording precisely because the rest of this section says the opposite.
