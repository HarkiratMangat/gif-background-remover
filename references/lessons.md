...
**Why it is not a full fix, traced rather than assumed.** The pixels that still jump were checked individually: `(41,204)`'s best single-ray fit is `res=6.61` (ceb7c3) — six times worse than `(41,203)`'s `res=1.08` (db4b86) — meaning no single ray explains `(204)` nearly as tightly as its neighbour does. That signature (a locally poor fit versus a locally excellent one right next door) is consistent with genuine antialiasing between TWO DIFFERENT DESIGN ELEMENTS (the pink fill touching the navy border) rather than within-family ambiguity a wider blend radius can smooth — a legitimate boundary between two things that are each fading independently, not a single ray rendered with a discontinuity. Whether the remaining ~2,100 jumps are mostly this (correct, unfixable by blending) or mostly a real remaining algorithmic gap was **not resolved this session**. The entry point for anyone continuing this is `unmix_against_palette`'s own single-nearest-line match (`k = r2.argmin(1)`) — a proper k-nearest or barycentric unmix, not another blend-radius or softmax-temperature tweak, and it needs the SAME real-adjacent-pixel tracing method used here (not a reconstruction) before trusting any conclusion about what is actually happening at a given border pixel.
